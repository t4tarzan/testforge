...
export default async function handler(req, res) {
  ...
  try {
    ...
    const rows = await db.execute(
      sql`SELECT * FROM reports WHERE test_run_id = ${reportId} LIMIT 1`
    );
    
    if (rows.length > 0) {
      // Also get findings for this run
      const findings = await db.execute(
        sql`SELECT * FROM findings WHERE test_run_id = ${reportId} ORDER BY severity, created_at`
      );
