  ...
  useEffect(() => {
    ...
    function draw() {
      ...
      for (let i = 0; i < nodes.length; i++) {
        for (let j = i + 1; j < nodes.length; j++) {
          ...
          if (dist < CONNECTION_DISTANCE) {
            ...
            ctx!.strokeStyle = `rgba(87, 74, 125, ${alpha})`
            ctx!.lineWidth = 0.8
            ctx!.stroke()
          }
        }
      ...
      for (const p of packets) {
        ...
        ctx!.fillStyle = `rgba(122, 111, 173, ${0.9 * (1 - p.progress)})`
        ctx!.fill()
      }
      ...
      for (const node of nodes) {
        ...
        glow.addColorStop(0, `rgba(87, 74, 125, ${0.2 * pulse})`)
        glow.addColorStop(1, 'rgba(87, 74, 125, 0)')
        ctx!.beginPath()
        ...
        ctx!.fillStyle = `rgba(87, 74, 125, ${0.7 + pulse * 0.3})`
        ctx!.fill()
      }
    }
