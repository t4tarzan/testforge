...
export const stages: Stage[] = [
  ...
  },
  {
    ...
    subtitle: 'Cyclomatic complexity + churn',
    description: 'Scores each file from AST-derived cyclomatic complexity, function length, dependency density, and optional git-churn signals. The output is a ranked heatmap so reviewers spend deep-review time on the files that earn it.',
    status: 'passed',
    metrics: { testsRun: 31, failures: 3, coverage: 91, duration: '22s' },
    logs: [
      '[INFO] Walking AST for complexity per function...',
      '[INFO] Cyclomatic threshold: 10 (McCabe)',
      '[INFO] Aggregating churn signals from git log...',
      '[WARN] High-risk module: auth/service.ts (complexity 24)',
      '[WARN] Medium-risk: payment/processor.ts (complexity 14)',
      '[INFO] Risk heatmap produced',
      '[FAIL] auth/service.ts — 3 functions exceed threshold',
      '[PASS] All other modules within threshold',
    ],
    findings: [
      { severity: 'critical', description: 'auth/service.ts has 3 functions above complexity 20', detail: 'High-complexity functions correlate with defect density. Consider extracting branches into named helpers.' },
      { severity: 'high', description: 'payment/processor.ts overflow risk', detail: 'Refund amount handling not bounded — overflow possible above MAX_SAFE_INTEGER.' },
    ],
  },
  {
    id: 7,
    number: '07',
    name: 'Security (SAST + Taint)',
    subtitle: 'Babel AST + taint flow, OWASP-mapped',
    description: 'Static security analysis. Traces tainted sources (req.params, req.body, req.query) through function calls to sensitive sinks (SQL, eval, exec, fs, innerHTML). Findings map to OWASP Top 10 categories.',
    status: 'running',
    metrics: { testsRun: 89, failures: 2, coverage: 98, duration: '38s' },
    logs: [
      '[INFO] Parsing files with Babel...',
      '[INFO] Indexing taint sources (req.body, req.params, req.query)...',
      '[INFO] Walking call graph for sink reachability...',
      '[WARN] Tainted input reaches db.query in /api/search',
      '[WARN] Hardcoded secret pattern detected: validator.ts:14',
      '[FAIL] CORS misconfig: origin "*" + credentials true',
      '[INFO] Mapping findings to OWASP Top 10...',
      '[PASS] OWASP coverage report generated',
    ],
    findings: [
  ...
  },
  {
    ...
    subtitle: 'CVE & vulnerability database',
    description: 'Cross-references all dependencies against known CVE databases. Flags vulnerable packages with severity ratings and recommended fix versions.',
    status: 'pending',
    metrics: { testsRun: 0, failures: 0, coverage: 0, duration: '--' },
    logs: ['[INFO] Scanning package.json...', '[INFO] 18 dependencies to check', '[INFO] Cross-referencing OSV database...', '[INFO] Found 2 known vulnerabilities', '...'],
    findings: [],
  },
  {
    id: 15,
    ...
    subtitle: 'Database performance analysis',
    description: 'Scans for database queries inside loops. Detects missing eager loading, unoptimized joins, and potential N+1 query patterns that cause performance degradation.',
    status: 'pending',
    metrics: { testsRun: 0, failures: 0, coverage: 0, duration: '--' },
    logs: ['[INFO] Scanning for ORM queries...', '[INFO] Analyzing loop structures...', '[INFO] Detected 3 potential N+1 patterns', '...'],
    findings: [],
  },
  {
    id: 16,
    ...
    subtitle: 'Unused code & dependency cleanup',
    ...
    status: 'pending',
    metrics: { testsRun: 0, failures: 0, coverage: 0, duration: '--' },
    logs: ['[INFO] Building import graph...', '[INFO] 5 unused dependencies found', '[INFO] 12 potentially dead exports', '...'],
    findings: [],
  },
  {
    id: 17,
    ...
    subtitle: 'Open source license audit',
    description: 'Checks all dependencies for license type. Flags GPL/copyleft risks. Ensures compliance with your organization&apos;s open source policy.',
    status: 'pending',
    metrics: { testsRun: 0, failures: 0, coverage: 0, duration: '--' },
    logs: ['[INFO] Checking licenses...', '[INFO] 18 packages audited', '[INFO] No copyleft risks detected', '...'],
    findings: [],
  },
  {
    id: 18,
    number: '18',
    name: 'DORA Metrics',
    subtitle: 'DevOps performance indicators',
    description: 'Estimates Deployment Frequency, Lead Time for Changes, Mean Time to Recovery, and Change Failure Rate from CI/CD configuration and code patterns.',
    status: 'pending',
    metrics: { testsRun: 0, failures: 0, coverage: 0, duration: '--' },
    logs: ['[INFO] Analyzing CI/CD config...', '[INFO] Estimating deployment frequency...', '[INFO] Lead time: < 1 day', '[INFO] MTTR: < 1 hour', '...'],
    findings: [],
  },
  {
    id: 19,
    number: '19',
    name: 'OWASP Coverage',
    subtitle: 'Top 10 security standards',
    description: 'Maps all security findings to OWASP Top 10 categories. Reports coverage percentage and identifies gaps in security testing coverage.',
    status: 'pending',
    metrics: { testsRun: 0, failures: 0, coverage: 0, duration: '--' },
