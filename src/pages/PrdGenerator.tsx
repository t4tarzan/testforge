      ...
      <div className="container-tf relative z-10 pt-[140px] pb-[80px] lg:pt-[160px] lg:pb-[100px]">
        ...
        <h1 className="text-display-xl text-[#333333] max-w-[800px] mb-6">
          {headlineWords.map((word, i) => {
            ...
            if (word === 'requirements.') color = 'text-[#C1A3FF]'
            return (
              ...
              <div className="prd-hero-step relative">
                ...
                <div
                  className={`w-10 h-10 rounded-full border-2 flex items-center justify-center ${
                    step.glow ? 'border-[#C1A3FF] shadow-[0_0_12px_rgba(90,143,94,0.4)]' : 'border-[#C1A3FF]'
                  }`}
                >
                  <step.icon size={18} className={step.glow ? 'text-[#C1A3FF]' : 'text-[#C1A3FF]'} />
                </div>
                <p className="text-label-mono text-[10px] mt-2 text-center text-[#6B6B6B]">{step.label}</p>
              </div>
              {i < 3 && (
                <ArrowRight size={18} className="prd-hero-arrow text-[#C9B5FF] -mt-5" />
              )}
            </div>
          ))}
        </div>
  ...
  return (
    <section ref={sectionRef} className="relative w-full bg-[#1A1A1A]" style={{ minHeight: '250vh' }}>
      ...
      <div className="sticky top-0 z-30 w-full h-[2px] bg-[#3A3A3A]">
        <div
          ref={progressRef}
          className="h-full bg-[#C1A3FF] transition-all duration-500 ease-out"
      ...
      <div className="container-tf relative z-10 py-20 lg:py-28">
        ...
        <h2 className="text-display-lg text-white mb-16">
          ...
          <span className="text-[#C1A3FF]">green</span> roadmaps.
        </h2>
              ...
              >
                ...
                <div className="flex flex-wrap gap-2">
                  {phases[activePhase].tags.map((tag) => (
                    <span
                      key={tag}
                      className="px-3 py-1 rounded text-[11px] font-mono font-medium uppercase tracking-wider border border-[#3A3A3A] text-[#C9B5FF]"
                    >
                      {tag}
                    </span>
                  ))}
              ...
              {phases.map((_, i) => (
                <button
                  ...
                  className={`h-1.5 rounded-full transition-all duration-300 ${
                    i === activePhase ? 'w-8 bg-[#C1A3FF]' : 'w-4 bg-[#3A3A3A] hover:bg-[#6B6B6B]'
                  }`}
                />
              ))}
            </div>
  ...
  return (
    <div className="bg-[#2A2A2A] border border-[#3A3A3A] rounded-xl p-6 shadow-2xl">
      ...
      <div className="mt-4 flex items-center gap-2">
        ...
        <span className="text-[#C1A3FF] text-xs font-mono">Root cause found in 0.8s</span>
      </div>
    </div>
  )
}
...
function SeverityVisual() {
  const severities = [
    ...
    { label: 'LOW', color: '#C1A3FF', desc: 'Minor issue' },
  ]
  ...
  return (
    <div className="bg-[#2A2A2A] border border-[#3A3A3A] rounded-xl overflow-hidden shadow-2xl">
      <div className="px-4 py-3 border-b border-[#3A3A3A] bg-[#1A1A1A] flex items-center justify-between">
        ...
        <span className="px-2 py-0.5 rounded text-[10px] font-mono bg-[rgba(90,143,94,0.15)] text-[#C1A3FF]">
          Generated
        </span>
      </div>
      ...
      <div className="px-4 py-2 border-t border-[#3A3A3A]">
        <span className="text-[10px] font-mono text-[#C1A3FF]">Generated by TestForge</span>
      </div>
    </div>
  )
}
...
function CheckMini() {
  return (
    <svg width="14" height="14" viewBox="0 0 14 14" fill="none">
      <circle cx="7" cy="7" r="6.5" stroke="#C1A3FF" strokeWidth="1" />
      <path d="M4 7L6 9L10 5" stroke="#C1A3FF" strokeWidth="1.2" strokeLinecap="round" strokeLinejoin="round" />
    </svg>
  )
}
    ...
    },
    {
      ...
      content: (
        <div className="space-y-2 text-sm">
          <p><span className="text-[#C1A3FF] font-mono">Phase 1:</span> Deploy token refresh endpoint</p>
          <p><span className="text-[#C1A3FF] font-mono">Phase 2:</span> Update client SDK</p>
          <p><span className="text-[#C1A3FF] font-mono">Phase 3:</span> Enable Redis session store</p>
          <p><span className="text-[#C1A3FF] font-mono">Phase 4:</span> Gradual rollout with feature flag</p>
        </div>
      ),
    },
    {
    ...
    <section ref={sectionRef} className="relative w-full bg-[#F5F5F0] py-24 lg:py-32">
      ...
      <div className="container-tf relative z-10">
        ...
        <h2 className="text-display-lg text-[#333333] mb-12">
          Every PRD follows a <span className="text-[#C1A3FF]">proven structure</span>.
        </h2>
        ...
        <div className="prd-preview-card max-w-[900px] mx-auto bg-white border border-[#D9D9D3] rounded-2xl shadow-[0_8px_32px_rgba(0,0,0,0.06)] overflow-hidden">
          ...
          <div className="px-6 py-4 border-b border-[#D9D9D3] flex flex-wrap items-center gap-3">
            ...
            <div className="ml-auto flex items-center gap-2">
              <span className="px-2.5 py-0.5 rounded text-[10px] font-mono font-medium bg-[#F0EAFF] text-[#C1A3FF]">
                Generated
              </span>
              <span
    ...
    <section ref={sectionRef} className="relative w-full bg-[#EBEBE5] py-20 lg:py-24">
      <div className="container-tf">
        ...
        <h2 className="text-display-md text-[#333333] mb-10">
          <span className="text-[#C1A3FF]">Priority</span>, not panic.
        </h2>
    ...
    <section ref={sectionRef} className="relative w-full bg-[#1A1A1A] py-20 lg:py-24">
      <div className="container-tf">
        <h2 className="text-display-md text-white text-center mb-14">
          From bug report to <span className="text-[#C1A3FF]">product roadmap</span>.
        </h2>
        ...
        <div className="flex flex-wrap justify-center gap-3 mt-14">
          {['Structured Requirements', 'Clear Priorities', 'Migration Paths', 'Faster Resolution'].map((pill) => (
            <span
              key={pill}
              className="px-5 py-2.5 rounded-lg border border-[#3A3A3A] text-[#C9B5FF] text-[13px] font-mono font-medium"
            >
              {pill}
            </span>
          ))}
